  ...
  const [users, setUsers] = useState<User[]>([]);
    ...
    const teams = Array.from({ length: 32 }, (_, index) => ({
      ...
      const filterPaidUsers = users.filter((user) => {
        return user.leagueRegistered.championsLeague === "Active";
      });
  return (
    <div>
      ...
      {filterPaidUsers.length > 0 ? (
        <>
      <h2 className='font-semibold text-lg'>Champions League</h2>
...
export default ChampionsTables